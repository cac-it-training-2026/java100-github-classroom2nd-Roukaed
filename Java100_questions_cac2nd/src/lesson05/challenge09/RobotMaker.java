...
class Robot {
	...
	void makeOmelet(int eggNum, int butterNum) {
		int OmeletNum = (int) eggNum / 2;
		if (OmeletNum * 5 > butterNum) {
			OmeletNum -= ((OmeletNum * 5) - (butterNum)) / 5;
		}
		System.out.println("オムレツを" + OmeletNum + "個作りました.\n");
	}
	...
	String makeEggDishes(int flourNum, int sugerNum, int eggNum, int butterNum) {
		if (flourNum >= 170 && sugerNum >= 50 && eggNum >= 0 && butterNum >= 80) {
			menu = "クッキー";
		} else if (eggNum >= 1 && butterNum >= 5) {
			menu = "オムレツ";
		} else if (eggNum >= 1) {
			menu = "ゆで卵";
		} else {
...
public class RobotMaker {
	...
	public static void main(String[] args) throws IOException {
		...
		String menu = robot.makeEggDishes(flourNum, butterNum, eggNum, butterNum);
